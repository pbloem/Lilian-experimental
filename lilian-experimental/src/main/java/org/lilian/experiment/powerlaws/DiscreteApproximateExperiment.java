...
public class DiscreteApproximateExperiment extends AbstractExperiment
{	
	...
	public @State PowerLaw.Fit<Integer, DiscreteApproximate> fit;
	...
	public DiscreteApproximateExperiment(
	...
	protected void body()
	{
		...
		fit = DiscreteApproximate.fit(data);
		cpl = fit.fit();
		...
		for(int i : series(bootstrapSize))
		{
			...
			// * fit model
			PowerLaw<Integer> cpl = DiscreteApproximate.fit(bData).fit();
	...
	{
		return DiscreteApproximate.fit(data).fit(12).exponent();
	}
